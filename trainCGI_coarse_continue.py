...
trainLoaderHelper = CGI(dataFolder = os.path.join(baseFolder, 'images'), 
	...
	fileListName=os.path.join(baseFolder, 'training_combine.list'),
	#fileListName=os.path.join(baseFolder, 'debug.list'),
	missingListName=os.path.join(baseFolder, 'missingData.list'),
...
testLoaderHelper = CGI(dataFolder=os.path.join(baseFolder, 'images'),
	...
	normalFolder=os.path.join(baseFolder, 'normal'),
	maskFolder=os.path.join(baseFolder, 'mask'),
	fileListName=os.path.join(baseFolder, 'validation.list'),
	missingListName=os.path.join(baseFolder, 'missingData.list'),
...
def networkForward(data, Testing=False):
    ...
    return loss, output_albedo, output_shading, output_normal, albedo, shading, normal, normalMask
...
def main(savePath, lr=1e-3, weight_decay=0, total_epoch=100):
    ...
    # load model
    my_network.load_state_dict(torch.load('result/result_CGI/result_coarse_lessData_0.0010_0.00_0100/trained_model/trained_model_91.t7'))
    ...
    for epoch in range(total_epoch):
        ...
        for i , data in enumerate(trainLoader, 0):
            ...
            loss_miniBatch, output_albedo, output_shading, output_normal,\
                    albedo, shading, normal, normalMask = networkForward(data, Testing=False)
            ...
            if i%monitor_count == monitor_count - 1:
                ...
                writer.add_image('train/normalMask', normal)
        ...
        for i, data in enumerate(testLoader, 0):
            loss_miniBatch, output_albedo, output_shading, output_normal,\
                    albedo, shading, normal, normalMask = networkForward(data, Testing=True)
            loss = loss_miniBatch['albedo'] + loss_miniBatch['albedo_grad'] + \
                    loss_miniBatch['shading'] + loss_miniBatch['shading_grad'] + \
                    loss_miniBatch['normal'] + loss_miniBatch['normal_grad'] + loss_miniBatch['light']
            numLoss = len(loss_miniBatch['loss_list'])
        ...
        writer.add_image('test/normal_mask', normal)
